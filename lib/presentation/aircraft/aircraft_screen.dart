...
import 'package:simplelog/state/providers/aircraft_controller_provider.dart';
...
class _AircraftScreenState extends ConsumerState<AircraftScreen> {
  ...
  Future<void> _showAircraftDetails(AircraftRow row) async {
    ...
    final entriesFuture = _loadAircraftLogEntries(row.aircraft.id);
  ...
  Future<List<LogbookEntry>> _loadAircraftLogEntries(int aircraftId) async {
    final db = ref.read(databaseProvider);
    ...
    final flightQuery = db.select(db.flights).join([
      ...
      ..where(db.flights.aircraftId.equals(aircraftId));
    ...
    final simQuery = db.select(db.simulatorTrainings).join([
      ...
      ..where(db.simulatorTrainings.aircraftId.equals(aircraftId));
