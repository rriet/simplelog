...
class LogbookRepository implements LogbookRepositoryContract {
  ...
  Future<void> toggleEntryLock(LogbookEntry entry) async {
    switch (entry.type) {
      case LogbookEventType.flight:
        ...
        if (item == null) return;
        await _db.update(_db.flights).replace(
      ...
      case LogbookEventType.simulatorTraining:
        ...
        if (item == null) return;
        await _db.update(_db.simulatorTrainings).replace(
              item.copyWith(isLocked: !item.isLocked),
            );
        return;
      case LogbookEventType.positioning:
        ...
        if (item == null) return;
        await _db.update(_db.positionings).replace(
              item.copyWith(isLocked: !item.isLocked),
            );
        return;
      case LogbookEventType.dutyPeriod:
        ...
        if (item == null) return;
        await _db.update(_db.dutyPeriods).replace(
              item.copyWith(isLocked: !item.isLocked),
            );
        return;
  ...
  Future<DutyPeriod?> findDutyById(int dutyId) async {
    return (_db.select(_db.dutyPeriods)..where((t) => t.id.equals(dutyId)))
        .getSingleOrNull();
  }

  Future<Flight?> findFlightById(int flightId) async {
    return (_db.select(_db.flights)..where((t) => t.id.equals(flightId)))
        .getSingleOrNull();
  }
  ...
  Future<FlightEditData?> loadFlightEditData(int flightId) async {
    ...
    final departureLine = await (_db.select(_db.timeLines)
          ..where((t) => t.id.equals(flight.departureDateTimeId)))
        .getSingleOrNull();
  ...
  Future<List<FlightCrewAssignment>> fetchFlightCrewAssignments(int flightId) {
    return (_db.select(_db.flightCrewAssignments)
  ...
  Future<DutyEditData?> loadDutyEditData(int dutyId) async {
    ...
    final startLine = await (_db.select(_db.timeLines)
          ..where((tbl) => tbl.id.equals(duty.dutyStartTimeLineId)))
        .getSingleOrNull();
    final endLine = await (_db.select(_db.timeLines)
  ...
  Future<Positioning?> findPositioningById(int positioningId) async {
    return (_db.select(_db.positionings)
          ..where((t) => t.id.equals(positioningId)))
        .getSingleOrNull();
  }

  Future<PositioningEditData?> loadPositioningEditData(int positioningId) async {
  ...
  Future<SimulatorTraining?> findSimulatorTrainingById(int simulatorId) async {
    return (_db.select(_db.simulatorTrainings)
          ..where((t) => t.id.equals(simulatorId)))
        .getSingleOrNull();
  }
  ...
  Future<List<SimulatorCrewAssignment>> fetchSimulatorCrewAssignments(
    ...
    return (_db.select(_db.simulatorCrewAssignments)
          ..where((t) => t.simulatorId.equals(simulatorId)))
        .get();
  }
  ...
  Future<void> createDuty({
    ...
    await _db.transaction(() async {
      final startId = await _db.into(_db.timeLines).insert(
            TimeLinesCompanion.insert(eventDateTime: start),
          );
      final endId = await _db.into(_db.timeLines).insert(
            TimeLinesCompanion.insert(eventDateTime: end),
          );
      await _db.into(_db.dutyPeriods).insert(
  ...
  Future<void> updateDuty({
    ...
    await _db.transaction(() async {
      ...
      final endLine = await (_db.select(_db.timeLines)
            ..where((tbl) => tbl.id.equals(duty.dutyEndTimeLineId)))
          .getSingle();
      await _db.update(_db.timeLines).replace(
            startLine.copyWith(eventDateTime: start),
          );
      await _db.update(_db.timeLines).replace(
            endLine.copyWith(eventDateTime: end),
          );
      await _db.update(_db.dutyPeriods).replace(
            duty.copyWith(
              timeDutyMinutes: dutyMinutes,
  ...
  Future<void> createPositioning({
    ...
    await _db.transaction(() async {
      ...
      await _db.into(_db.positionings).insert(
  ...
  Future<void> createFlight({
    ...
    await _db.transaction(() async {
      ...
      final flightId = await _db.into(_db.flights).insert(
            FlightsCompanion.insert(
              aircraftId: aircraftId,
              departureAirportId: departureAirportId,
              arrivalAirportId: arrivalAirportId,
              departureDateTimeId: departureTimelineId,
              takeOffDateTime: Value(takeOffDateTime),
              landingDateTime: Value(landingDateTime),
              arrivalDateTime: Value(arrivalDateTime),
              ...
              approachType: approachType.isEmpty ? pilotFunction : approachType,
              remarks: remarks,
              notes: notes,
              isLocked: false,
      ...
      await _replaceFlightCrewAssignments(flightId, crewAssignments);
    });
  }
  ...
  Future<void> updateFlight({
    ...
    await _db.transaction(() async {
      final departureLine = await (_db.select(_db.timeLines)
          ...
          .getSingle();
      await _db.update(_db.timeLines).replace(
            departureLine.copyWith(eventDateTime: departureDateTime),
          );
      await _db.update(_db.flights).replace(
            flight.copyWith(
              aircraftId: aircraftId,
      ...
      await _replaceFlightCrewAssignments(flight.id, crewAssignments);
    });
  }
  ...
  Future<void> createSimulatorTraining({
    ...
    await _db.transaction(() async {
      final startTimelineId = await _db.into(_db.timeLines).insert(
            TimeLinesCompanion.insert(eventDateTime: startDateTime),
          );
      final simulatorId = await _db.into(_db.simulatorTrainings).insert(
  ...
  Future<void> updateSimulatorTraining({
    ...
    await _db.transaction(() async {
      final startLine = await (_db.select(_db.timeLines)
          ...
          .getSingle();
      await _db.update(_db.timeLines).replace(
            startLine.copyWith(eventDateTime: startDateTime),
          );
      await _db.update(_db.simulatorTrainings).replace(
  ...
  Future<void> _replaceSimulatorCrewAssignments(
    ...
    await (_db.delete(_db.simulatorCrewAssignments)
          ..where((t) => t.simulatorId.equals(simulatorId)))
        .go();
  ...
  Future<void> _replaceFlightCrewAssignments(
    ...
    await (_db.delete(_db.flightCrewAssignments)
          ..where((t) => t.flightId.equals(flightId)))
        .go();
    if (assignments.isEmpty) return;
    for (final assignment in assignments) {
  ...
  Future<List<String>> fetchSimulatorCrewLabels(int simulatorId) async {
    final rows = await (_db.select(_db.simulatorCrewAssignments).join([
      innerJoin(
        _db.crew,
        _db.crew.id.equalsExp(_db.simulatorCrewAssignments.crewId),
      ),
    ])..where(_db.simulatorCrewAssignments.simulatorId.equals(simulatorId)))
        .get();
    return rows.map((row) {
      final crew = row.readTable(_db.crew);
  ...
  Future<List<LogbookEntry>> fetchEntriesForAirport(int airportId) async {
    ...
    final positioningQuery = _db.select(_db.positionings).join([
      innerJoin(
        _db.timeLines,
  ...
  Future<List<LogbookEntry>> fetchEntriesForAircraft(int aircraftId) async {
    ...
    final flightQuery = _db.select(_db.flights).join([
      ...
      leftOuterJoin(_db.aircrafts, _db.aircrafts.id.equalsExp(_db.flights.aircraftId)),
      leftOuterJoin(
        _db.aircraftTypes,
        _db.aircraftTypes.id.equalsExp(_db.aircrafts.aircraftTypeId),
      ),
  ...
  Future<List<LogbookEntry>> fetchEntriesForCrew(int crewId) async {
    ...
    final simQuery = _db.select(_db.simulatorTrainings).join([
      innerJoin(
        ...
        _db.simulatorCrewAssignments.simulatorId
            .equalsExp(_db.simulatorTrainings.id),
      ),
      innerJoin(
        _db.timeLines,
        _db.timeLines.id.equalsExp(_db.simulatorTrainings.startTimeLineId),
  ...
  Future<void> deleteEntry(LogbookEntry entry) async {
    switch (entry.type) {
      case LogbookEventType.flight:
        ...
        await _db.transaction(() async {
          await (_db.delete(_db.flightCrewAssignments)
              ...
              .go();
          await (_db.delete(_db.flights)..where((t) => t.id.equals(flight.id)))
              .go();
          await (_db.delete(_db.timeLines)
                ..where((t) => t.id.equals(flight.departureDateTimeId)))
              .go();
        });
        return;
      case LogbookEventType.simulatorTraining:
        ...
        await _db.transaction(() async {
          await (_db.delete(_db.simulatorCrewAssignments)
                ..where((t) => t.simulatorId.equals(sim.id)))
              .go();
          await (_db.delete(_db.simulatorTrainings)
                ..where((t) => t.id.equals(sim.id)))
              .go();
          await (_db.delete(_db.timeLines)
                ..where((t) => t.id.equals(sim.startTimeLineId)))
              .go();
        });
        return;
      case LogbookEventType.positioning:
        ...
        await _db.transaction(() async {
          await (_db.delete(_db.positionings)
                ..where((t) => t.id.equals(positioning.id)))
              .go();
          await (_db.delete(_db.timeLines)
                ..where((t) => t.id.equals(positioning.departureDateTimeId)))
              .go();
  ...
  Future<void> deleteDutyById(int dutyId) async {
    ...
    if (duty == null) return;
    await _db.transaction(() async {
      await (_db.delete(_db.dutyPeriods)..where((t) => t.id.equals(duty.id))).go();
